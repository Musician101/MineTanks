 ...
 * @note Original class name is Cuboid
 */
public class CuboidUtil implements Iterable<Block>, ConfigurationSerializable {
    private String worldName = "";
    private int x1 = 0, y1 = 0, z1 = 0;
    private int x2 = 0, y2 = 0, z2 = 0;
 
    public CuboidUtil(Location location) {
        this(location, location);
    }
 
    public CuboidUtil(Location location, Location location2) {
    ...
    private CuboidUtil(Map<String, Object> serializedCuboid) {
    ...
    // Method added by Musician101
    // Y check is ignored as it's not needed
    public boolean isInCuboid(Location location)
    {
    	return !(location.getX() <= x1 || location.getX() >= x2 || location.getZ() <= z1 || location.getZ() >= z2);
    }
    
    public static CuboidUtil deserialize(Map<String, Object> serializedCuboid) {
        return new CuboidUtil(serializedCuboid);
    }
 
    public static CuboidUtil createFromLocationRadius(Location location, double radius) {
        return createFromLocationRadius(location, radius, radius, radius);
    }
 
    public static CuboidUtil createFromLocationRadius(Location location, double xRadius, double yRadius, double zRadius) {
        Validate.notNull(location);
        if (xRadius < 0 || yRadius < 0 || zRadius < 0)
            throw new IllegalArgumentException("The radius cannot be negative!");
        return xRadius > 0 || yRadius > 0 || zRadius > 0 ? new CuboidUtil(location.clone().subtract(xRadius, yRadius, zRadius), location.clone().add(xRadius, yRadius, zRadius)) : new CuboidUtil(location);
    }
}
