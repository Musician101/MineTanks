...
public class StatusSpigotCommand extends AbstractSpigotCommand<SpigotMineTanks>
{
    public StatusSpigotCommand(SpigotMineTanks plugin)
    {
        super(plugin, CommonCommands.STATUS_NAME, CommonCommands.STATUS_DESC, new SpigotCommandUsage(Arrays.asList(new SpigotCommandArgument(CommonCommands.MT_CMD), new SpigotCommandArgument(CommonCommands.STATUS_NAME), new SpigotCommandArgument(CommonCommands.FIELD, Syntax.REPLACE, Syntax.REQUIRED)), 1), new SpigotCommandPermissions(CommonPermissions.EDIT_PERM, false, ChatColor.RED + CommonMessages.NO_PERMISSION, ChatColor.RED + CommonMessages.PLAYER_ONLY));
    }
