...
public class RedSpawnSpigotCommand extends AbstractSpigotCommand<SpigotMineTanks>
{
    public RedSpawnSpigotCommand(SpigotMineTanks plugin)
    {
        super(plugin, CommonCommands.RED_SPAWN_NAME, CommonCommands.RED_SPAWN_DESC, new SpigotCommandUsage(Arrays.asList(new SpigotCommandArgument(CommonCommands.MT_CMD), new SpigotCommandArgument(CommonCommands.RED_SPAWN_NAME), new SpigotCommandArgument(CommonCommands.FIELD, Syntax.REPLACE, Syntax.REQUIRED)), 1), new SpigotCommandPermissions(CommonPermissions.EDIT_PERM, true, ChatColor.RED + CommonMessages.NO_PERMISSION, ChatColor.RED + CommonMessages.PLAYER_ONLY));
    }
