...
import io.musician101.minetanks.sponge.handler.ExplosionTracker;
import io.musician101.minetanks.sponge.handler.ReloadHandler;
...
public class MTListener
{
    ...
    public void onBlockInteract(Secondary event)
    {
        ...
        for (String name : plugin.getFieldStorage().getFields().keySet())
        {
            SpongeBattleField field = plugin.getFieldStorage().getField(name);
            if (field.inProgress())
                return;
    ...
    public void onPlayerJoin(Join event)
    {
        ...
        SpongeInventoryStorage sis = plugin.getInventoryStorage();
    ...
    public void onPlayerDisconnect(Disconnect event)
    {
        ...
        for (String name : plugin.getFieldStorage().getFields().keySet())
        {
            if (plugin.getFieldStorage().getField(name).getPlayerTank(player.getUniqueId()) != null)
            {
                plugin.getFieldStorage().getField(name).removePlayer(player.getUniqueId());
                return;
            }
        }
    }
    ...
    public void onBowShoot(LaunchProjectileEvent event)
    {
        ...
        for (String name : plugin.getFieldStorage().getFields().keySet())
        {
            ...
            if (field.getPlayerTank(player.getUniqueId()) != null)
            {
                ...
                ReloadHandler reload = new ReloadHandler(plugin, player, cannon, pt.getClipSize());
